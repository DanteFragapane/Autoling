...
function randCuddle(name) {
  ...
  `Ling gently picks up ${name} and cradles you, rocking softly.`
  ];
...
function chooseRand(msg) {
  ...
  let onlineMemb = members.filter(m => m.presence.status === 'online' && m.user.bot == false).keyArray();
  console.dir(onlineMemb.length);
  if(onlineMemb.length == 0) return "none";
  let isBot = true;
  while(isBot) {
    let randUsrID = membArray[Math.floor(Math.random() * membArray.length)];
  	let randUsr = members.get(randUsrID).user;
    if(randUsrID.presence == 'offline') return chooseRand(msg);
	  switch (randUsr.bot) {
	    case false:
        isBot=false;
        if(_.contains(cuddleBlacklist, randUsrID)) break;
		// return randUsr.randUsernameme;
        // return randUsrID; 
        return randUsr;
	    case true:
		    chooseRand(msg);
...
bot.on('message', msg => {
  ...
  if(msg.channel.id == 272886608825548801 && msg.author.id != 255322858434199552) {
    ...
    switch (command) {
      ...
      case 'cuddle':
        if(params[0] == null) {
        	msg.reply("this command must have a parameter.");
          break;
        }
      ...
      case 'give':
       ...
       break;
  	  ...
  	  case 'rc':
  	  case 'randomCuddle':
  	   let randUser = chooseRand(msg);
  	   if (randUser == 'none') {msgChan.sendMessage("None found.")}
  	   else {msgChan.sendMessage(randCuddle(randUser))}
  	   break;
...
bot.on('ready', () => {
  ...
  console.log('I am Autoling! I cannot wait to cuddle with you!');
});
